...
import {
  ...
  CLEAR_ENTITIES,
  SET_DOOR_ENTITIES,
  SET_WEATHER_ENTITIES,
  SET_HOMEPOD_ENTITIES,
  SET_LOADING,
} from './types'
...
const EntitiesState = props => {
  ...
  const setDoorEntities = entities => {
    const doorIsOpen =
      entities[ENTITY_KEYS.DOORS.FRIDGE].state === 'on' ||
      entities[ENTITY_KEYS.DOORS.GARAGE].state === 'on'
    const doors = {
  ...
  const setEntities = entities => {
    ...
    setWeatherEntities(
      entities[ENTITY_KEYS.WEATHER.HOME],
      entities[ENTITY_KEYS.WEATHER.SUN]
    )
